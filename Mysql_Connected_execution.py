...
class mysql(object):
	...
	def __init__(self, host, port, account, password, source, output):
	...
	def initiate(self):
		try:
			...
			self.db = pymysql.connect(host=self.host, port=self.port, user=self.account, password=self.pw, charset='UTF8MB4', local_infile=True)
	...
	def export_db(self, table, filename):
		if os.path.exists('%s/%s' % (self.output,filename)):
			os.remove('%s/%s' % (self.output,filename))
	...
	def export_siteva(self):
		self.cursor.execute("use expert_system")
		self.export_db('sitevaluation', '%s/SiteEva_Table.csv' % self.dataset)
		...
		# if os.path.exists('%s/SiteEva_Table.csv' % self.output):
		# 	os.remove('%s/SiteEva_Table.csv' % self.output)
	...
	def export_water(self):
		self.cursor.execute("use expert_system")
		self.export_db('watering', '%s/Planting_Water.csv'%self.dataset)


	# 匯出土壤速測紀錄表格
	def export_soilstest(self):
		self.cursor.execute("use expert_system")
		self.export_db('soilstest', '%s/Soils_Test.csv'%self.dataset)
	...
	def export_fert(self):
		...
		for t in tables:
			...
			self.export_db(t, '%s/Planting_Fertilizer_%s.csv' % (self.dataset,t.replace('_','-')))
	...
	def import_siteva(self):
		self.import_db('expert_system', 'sitevaluation', '%s/SiteEva_Table_Update.csv'%self.dataset)
		self.db.commit()
		os.remove('%s/SiteEva_Table_Update.csv'%self.dataset)
	...
	def import_fert(self):
		results = glob.glob('%s/Planting_Fertilizer_*.csv'%self.dataset)
		for r in results:
			r = r.replace('\\','/')
	...
	def import_water(self):
		filename = self.edit_anchor('%s/Planting_water.csv'%self.dataset)
	...
	def import_soils(self):
		filename = self.edit_anchor('%s/Soils_Test.csv'%self.dataset)
	...
	def code_rearrange(self):
		...
		pc_df = pd.read_csv('%s/SiteEva_Table.csv'%self.dataset, header=0)
		...
		pc_df.to_csv('%s/SiteEva_Table_Update.csv'%self.dataset,encoding='utf_8_sig',index=False)
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(description='MySQL connected setting')
	parser.add_argument('--host', '-n', type=str, required=True,
						help='a host name for mysql connection')
	parser.add_argument('--port', '-p', type=int, required=True,
						help='a port name for mysql connection')
	parser.add_argument('--account', '-a', type=str, required=True,
						help='an account name for mysql connection')
	parser.add_argument('--password', '-pw', type=str, required=True,
						help='a port name for mysql connection')
	parser.add_argument('--source', '-s', type=str, default='./Dataset',
						help='path of datasets')
	parser.add_argument('--output', '-o', type=str, default=os.getcwd().replace('\\','/'),
						help='path of output')
	args = parser.parse_args()
	
	# print(args.host, args.port, args.account, args.password, args.source, args.output)
	sql_conc = mysql(args.host, args.port, args.account, args.password, args.source, args.output)
	sql_conc.upload()
	sql_conc.download()
